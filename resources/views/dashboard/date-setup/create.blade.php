                        ...
                        <div class="card">
                            <div class="body">
                                ...
                                <div class="tab-content">
                                    <form method="POST" action="{{route('admin.date-setup.store')}}"
                                          class="form-horizontal">
                                        @csrf

                                        <div class="form-group">
                                            <label for="Phone" class="col-sm-2 control-label">Date Name</label>
                                            <div class="col-sm-10">
                                                <div class="form-line">
                                                    <input type="text" class="form-control" name="name"
                                                           placeholder="Date Name" value="{{old('name')}}">
                                                </div>
                                            </div>
                                        </div>
                                                ...
                                                <div class="select">
                                                    ...
                                                    <select id="reminder" name="reminder_start" onChange="return updateSetReminder()">
                                                        <option selected="selected" value="">Select Reminder time</option>
                                                            <option  data-live="1" value="{{\App\Models\DateSetup::REMINDER_DAYS}}" >Days</option>
                                                            <option data-live="2" value="{{\App\Models\DateSetup::REMINDER_WEEKS}}" >Weeks</option>
                                                            <option data-live="3" value="{{\App\Models\DateSetup::REMINDER_MONTHS}}" >Months</option>
                                                            <option data-live="4" value="{{\App\Models\DateSetup::REMINDER_YEARS}}" >Years</option>
                                                    </select>
                                                    <label style="margin-left: 50px">Enter Number </label>
                                                    <input style="width:50px" type="number" min="1" name="number">
                                                    <input style="width: 60px;border: none" type="text" id="Reminder-Select" />
                                                </div>
                                            </div>
                                        </div>
